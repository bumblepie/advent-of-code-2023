...
impl NumberPosition {
    fn adjacent(&self, symbol_position: &Point) -> bool {
        ...
        return i64::abs(self.row as i64 - symbol_position.row as i64) < 2
            && column_values.into_iter().any(|column| i64::abs(column as i64 - symbol_position.column as i64) < 2);
    }
}
...
fn parse_line(line_number: usize, line: &str) -> (HashMap<NumberPosition, u64>, HashMap<Point, char>) {
    let numbers_regex: &Lazy<Regex> = regex!(r#"\d+"#);
    let numbers = numbers_regex.find_iter(line)
        .map(|m| {
            let range = m.range();
            let number = u64::from_str(m.as_str()).expect(&format!("Unable to parse number {}", m.as_str()));
    ...
    let symbols = symbols_regex.find_iter(line)
        .map(|m| {
            // Assumption: regex should match exactly one character -> match should be exactly one character long
            let position = Point { row: line_number, column: m.range().start};
...
fn parse_input(lines: impl Iterator<Item = String>) -> Schematic {
    lines
        ...
        .fold(Schematic::new(), |mut schematic, (next_numbers, next_symbols)| {
...
fn part_one(file_name: &str) -> u64
{
    ...
    schematic.numbers.iter()
        .filter(|(number_position, _number)| schematic.symbols.keys().any(|symbol_point| number_position.adjacent(symbol_point)))
        .map(|(_number_position, number)| number)
        .sum()
}
...
mod test {
    ...
    fn test_number_position_adjacency(#[case] number_position: NumberPosition, #[case] symbol_position: Point, #[case] expected_result: bool) {
